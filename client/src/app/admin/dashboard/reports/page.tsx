  ...
  return (
    <div className="space-y-6 pb-10 max-w-4xl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: '#0F172A', letterSpacing: '-0.025em' }}>Reports</h1>
          <p className="text-sm mt-1" style={{ color: '#94A3B8' }}>View session reports for each exam.</p>
        </div>
        <button
          onClick={fetchExams}
      ...
      <div className="relative max-w-sm">
        ...
        <input
          ...
          style={{ borderColor: '#E2E8F0', color: '#0F172A', paddingLeft: '2.5rem', paddingRight: '0.75rem' }}
          ...
          {filtered.map((exam) => {
            ...
            return (
              <div
                ...
                onMouseEnter={e => (e.currentTarget as HTMLElement).style.boxShadow = '0 4px 12px rgba(15,23,42,0.08)'}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 text-xs flex-wrap" style={{ color: '#94A3B8' }}>
                      <span className="flex items-center gap-1">
                        <Calendar className="h-3 w-3" /> {fmtDate(exam.start_window)}
                      </span>
                      <span className="flex items-center gap-1">
                        <Clock className="h-3 w-3" /> {exam.duration_minutes} min
                      </span>
                      <span className="flex items-center gap-1">
                        <Users className="h-3 w-3" /> {exam.invite_count} student{exam.invite_count !== 1 ? 's' : ''}
                      </span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-xs font-semibold hidden sm:block" style={{ color: '#22577A' }}>View Reports</span>
