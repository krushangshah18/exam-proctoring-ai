...
function InfoField({
  ...
  return (
    ...
    >
      ...
      <div className="text-sm font-semibold break-words" style={{ color: '#0F172A' }}>
        {value}
      </div>
    </div>
  );
...
export default function AdminSettingsPage() {
  ...
  const passwordChecks = useMemo(() => {
    ...
    return {
      ...
      color:
        score >= 4 ? '#15803D' : score >= 3 ? '#0369A1' : score >= 2 ? '#B45309' : '#DC2626',
      bg:
        score >= 4 ? '#ECFDF5' : score >= 3 ? '#F0F9FF' : score >= 2 ? '#FFFBEB' : '#FEF2F2',
  ...
  return (
    <div className="max-w-6xl space-y-6 pb-12">
      <div
        ...
        style={{ background: 'linear-gradient(135deg, #22577A 0%, #2C7DA0 48%, #57CC99 100%)' }}
      >
        <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 max-w-xl text-sm sm:text-base" style={{ color: 'rgba(255,255,255,0.78)' }}>
              Review your profile details, confirm account activity, and update your password from one place.
            </p>
          </div>
          ...
          >
            <div>
              ...
              <p className="mt-1 text-sm font-semibold text-white">{profile?.role ?? 'ADMIN'}</p>
            </div>
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em]" style={{ color: 'rgba(255,255,255,0.62)' }}>
                Last Login
              ...
              <InfoField
                ...
                value={
                  <span className="inline-flex items-center gap-2">
                    <ShieldCheck className="h-4 w-4" style={{ color: '#22577A' }} />
                    {profile?.role ?? '—'}
                  </span>
                }
                icon={ShieldCheck}
            ...
            <form onSubmit={handleChangePassword} className="space-y-5">
                  ...
                  <div className="relative">
                    <input
                      ...
                      onFocus={(e) => {
                        ...
                        (e.target as HTMLElement).style.boxShadow = '0 0 0 4px rgba(56,163,165,0.12)';
                      }}
                      onBlur={(e) => {
              ...
              >
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  {[
                    ...
                    >
                      <CheckCircle2 className="h-4 w-4 shrink-0" style={{ color: item.ok ? '#16A34A' : '#CBD5E1' }} />
                      <span className="text-sm" style={{ color: item.ok ? '#166534' : '#64748B' }}>
                        {item.label}
                      </span>
                    </div>
                  ))}
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-sm leading-6" style={{ color: '#64748B' }}>
                  Save only when the new password is complete and confirmed. Your current session will remain active.
                </p>
                <button
                  type="submit"
                  ...
                  style={{
                    background: savingPw || !passwordReady ? '#94A3B8' : '#22577A',
                    cursor: savingPw || !passwordReady ? 'not-allowed' : 'pointer',
                    boxShadow: savingPw || !passwordReady ? 'none' : '0 12px 28px rgba(34,87,122,0.22)',
                  }}
                >
