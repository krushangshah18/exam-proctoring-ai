  ...
  if (loading || !exam) {
    return (
      <div className="flex flex-col items-center justify-center p-24">
        <Loader2
          className="h-7 w-7 animate-spin mb-3"
          style={{ color: "#CBD5E1" }}
        />
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col md:flex-row items-start gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            onClick={() =>
              router.push(`/admin/dashboard/exams/${examId}/reports`)
            }
          ...
          {(exam.status === "LIVE" || exam.status === "ENDED") && (
            <button
              onClick={() =>
                router.push(`/admin/dashboard/exams/${examId}/monitor`)
              }
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold border transition-all duration-150"
              style={{
                borderColor: "#BFDBFE",
        ...
        {[
          {
            ...
            iconBg: "#ECFDF5",
            iconColor: "#15803D",
            value: fmtDate(exam.start_window),
            sub: fmtTimeTZ(exam.start_window),
          },
          {
            label: "End Window",
            icon: Calendar,
            iconBg: "#FFF1F2",
            iconColor: "#BE123C",
            value: fmtDate(exam.end_window),
            sub: fmtTimeTZ(exam.end_window),
          },
          {
            label: "Duration",
            icon: Clock,
          ...
          },
          {
            ...
            iconBg: "#FFFBEB",
            iconColor: "#B45309",
            value: `${exam.invites.length}`,
            sub: `${exam.invites.filter((i) => i.used).length} accessed`,
            bold: true,
          },
      ...
      >
        ...
        <div className="p-5">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-8 gap-y-5">
            {[
              {
                label: "Hard Join Deadline",
                value: fmtDateTime(exam.hard_join_deadline),
              },
              {
                label: "Late Join Policy",
                value: exam.late_join_policy.toLowerCase().replace("_", " "),
              },
            ].map(({ label, value }) => (
              <div key={label}>
                <p
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                >
                  {exam.allow_late_extension
                    ? `Allowed (up to ${exam.max_late_minutes} min)`
                    : "Not allowed"}
                </span>
              </div>
            </div>
            ...
            {(() => {
              ...
              for (const [key, enabled] of Object.entries(
                ...
                if (!enabled) {
                  ...
                  if (mapped)
                    disabledKeys.add(`${mapped.group}|${mapped.label}`);
                }
              }
              ...
              for (const [key, enabled] of Object.entries(allConfig)) {
                ...
                const mapped = KEY_MAP[key];
                if (!mapped) continue;
                if (disabledKeys.has(`${mapped.group}|${mapped.label}`))
                  continue;
                  ...
                  {active.map((group) => (
                    ...
                    >
                      <span
                        ...
                        style={{ color: "#475569", minWidth: "64px" }}
                      >
                        {group}
                      </span>
            ...
            <table className="w-full text-sm text-left">
              ...
              >
                <tr>
                  ...
                  <th
                    className="px-4 py-2.5 text-xs font-semibold text-center w-24"
                    style={{ color: "#64748B" }}
                  >
                    Status
                  </th>
