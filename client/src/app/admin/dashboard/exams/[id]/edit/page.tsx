...
const examSchema = baseExamSchema
  .refine(
    (data) => {
  ...
  .refine(
    (data) => {
      ...
      return (
        d.getTime() - start.getTime() >= 60000 && d <= new Date(data.end_window)
      );
    },
    {
      message:
        "Join deadline must be at least 1 minute after start time and before the end window",
...
const PRESETS = {
  ...
  MEDIUM: {
    ...
    detection: {
      DETECT_LOOKING_AWAY: false,
      DETECT_LOOKING_DOWN: false,
      DETECT_LOOKING_UP: false,
      DETECT_LOOKING_SIDE: true,
      DETECT_FACE_HIDDEN: true,
      DETECT_PARTIAL_FACE: true,
      DETECT_FAKE_PRESENCE: true,
...
export default function EditExamPage() {
  ...
  const form = useForm<ExamFormValues>({
    resolver: zodResolver(
      examSchema,
    ) as import("react-hook-form").Resolver<ExamFormValues>,
    defaultValues: {
      title: "",
      exam_mode: "FLEXIBLE",
      start_window: "",
  ...
  if (startWindow && durationMinutes) {
    ...
    if (!isNaN(s.getTime()))
      minEndString = toLocalInputString(
        new Date(s.getTime() + Number(durationMinutes) * 60000),
      );
  }
  ...
  const showDurationWarning = () => {
    ...
    return (
      !isNaN(hd) && !isNaN(end) && hd + Number(durationMinutes) * 60000 > end
    );
  };
  ...
  const addEmails = (raw: string) => {
    ...
    const unique = Array.from(
      new Set([...emails, ...found.map((e) => e.toLowerCase())]),
    );
    setEmails(unique);
  ...
  async function onSubmit(values: ExamFormValues) {
    ...
    try {
      ...
      toast.error(
        (error as ApiError).response?.data?.detail || "Failed to update exam",
      );
    } finally {
  ...
  async function handleAddStudentsOnly() {
    ...
    setIsSubmitting(true);
    try {
      const res = await api.post(`/admin/exams/${examId}/students`, {
        invites: emails,
      });
      toast.success(
        res.data?.message ||
          "Students added successfully. Invite emails are being sent.",
      );
      router.push(`/admin/dashboard/exams/${examId}`);
    } catch (error: unknown) {
      toast.error(
        (error as ApiError).response?.data?.detail || "Failed to add students",
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  if (lockReason) {
    const lockMessages: Record<
      NonNullable<LockReason>,
      { title: string; body: string }
    > = {
      time: {
        title: "Exam Locked",
        body: "Core exam settings are locked now, but you can still add missing students and the normal invite email flow will still be sent.",
      },
      ended: {
        title: "Exam Completed",
        body: "This exam has ended. Completed exam records cannot be modified.",
      },
      cancelled: {
        title: "Exam Cancelled",
        body: "This exam has been cancelled and cannot be edited.",
      },
    };
        ...
        <div className="max-w-4xl mx-auto space-y-6 pb-20">
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm mt-0.5" style={{ color: "#64748B" }}>
                {examTitle || "Exam"} · schedule and proctoring rules are locked
              </p>
            </div>
          </div>
          ...
          <FormSection
            icon={
              ...
              >
                <Users className="h-4 w-4" style={{ color: "#22577A" }} />
              </div>
            }
          ...
          >
            <div className="space-y-4">
              ...
              {emails.length > 0 && (
                <div
                  className="rounded-xl border p-4 max-h-60 overflow-y-auto"
      ...
      <div className="p-24 max-w-xl mx-auto text-center space-y-4">
        ...
        <div className="pt-6">
          <button
            ...
            style={{ background: "#22577A" }}
            onMouseEnter={(e) =>
              ((e.currentTarget as HTMLElement).style.background = "#1a4560")
            }
            onMouseLeave={(e) =>
              ((e.currentTarget as HTMLElement).style.background = "#22577A")
            }
          >
            Go Back
          </button>
    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-20">
      ...
      >
        ...
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: "#64748B" }}>
            Modify timing, proctoring rules, and manage attendees.
          </p>
        </div>
      </div>
          ...
          <FormSection
            icon={
              <div
                ...
                style={{ background: "#EFF6FF" }}
              >
                <FileText className="h-4 w-4" style={{ color: "#22577A" }} />
              </div>
            }
            title="Basic Details"
          >
                ...
                render={({ field }) => (
                  <FormItem className="col-span-1 md:col-span-2">
                    ...
                    <FormControl>
                      <input
                        type="text"
                        placeholder="e.g. CS401 Midterm Examination"
                        {...field}
                        className={inputBase}
                        style={inputStyle}
                        onFocus={inputFocus}
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        <SelectItem value="FLEXIBLE">
                          Flexible — anytime within window
                        </SelectItem>
                        <SelectItem value="FIXED">
                          Fixed — strict start &amp; end times
                        </SelectItem>
                      </SelectContent>
                    </Select>
                    <FormMessage />
                    ...
                    <FormControl>
                      <input
                        ...
                        style={{
                          ...inputStyle,
                          background:
                            examMode === "FIXED" ? "#F8FAFC" : undefined,
                          opacity: examMode === "FIXED" ? 0.6 : 1,
                        }}
                        onFocus={inputFocus}
                        onBlur={(e) => {
                          field.onBlur();
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <input
                        ...
                        style={{
                          ...inputStyle,
                          background:
                            examMode === "FLEXIBLE" ? "#F8FAFC" : undefined,
                          opacity: examMode === "FLEXIBLE" ? 0.7 : 1,
                        }}
                        onFocus={inputFocus}
                        onBlur={(e) => {
                          field.onBlur();
                    ...
                    {examMode === "FLEXIBLE" ? (
                      <FormDescription>
                        Auto-calculated: end window − duration.
                      </FormDescription>
                    ) : (
                      <FormDescription>
                        Latest time students may join the exam.
                      </FormDescription>
                    )}
                    <FormMessage />
                  </FormItem>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p
                        ...
                        style={{ color: "#78350F" }}
                      >
                        Students joining near the deadline will not get their
                        full duration before the window closes.
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <FormItem>
                    ...
                    >
                      <FormControl>
                        <SelectTrigger
                          style={
                            examMode === "FIXED" ? { opacity: 0.6 } : undefined
                          }
                        >
                          <SelectValue />
                        </SelectTrigger>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        Absolute cutoff: this many minutes after exam start
                        time.
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      {(["STRICT", "MEDIUM", "RELAXED", "CUSTOM"] as const).map(
                        (p) => (
                          <button
                            key={p}
                            type="button"
                            ...
                            style={{
                              background:
                                field.value === p ? "#57CC99" : "#fff",
                              color: field.value === p ? "#0F172A" : "#475569",
                              borderColor:
                                field.value === p ? "#57CC99" : "#E2E8F0",
                            }}
                          >
                            {p}
                          </button>
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <p
                    ...
                    style={{ color: "#15803D" }}
                  >
                    <span className="font-semibold">Always enabled:</span> Face
                    Hidden &amp; Partial Face — required for presence
                    verification.
                  </p>
                </div>

                <div
                  className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-5 rounded-xl border"
                  style={{ background: "#F8FAFC", borderColor: "#E2E8F0" }}
                >
                  {(
                    [
                      ...
                      {
                        id: "DETECT_FAKE_PRESENCE",
                        label: "Static Photo / Spoof",
                      },
                      ...
                      {
                        id: "DETECT_MULTIPLE_PEOPLE",
                        label: "Multiple People",
                      },
                    ] as { id: DetectionFieldKey; label: string }[]
                  ).map((item) => (
                    <FormField
                      key={item.id}
                      control={form.control}
                      name={`detection_config.${item.id}`}
                      render={({ field }) => (
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        Accumulated warnings before admin is notified.
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-80 p-4">
                      ...
                      <div
                        className="rounded-lg p-2 text-xs font-mono whitespace-pre-wrap"
