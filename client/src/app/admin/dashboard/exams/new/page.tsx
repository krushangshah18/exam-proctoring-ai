...
// ─── Timezone helpers ────────────────────────────────────────────────────────
// datetime-local inputs always work in the browser's local time.
// toLocalInputString converts a Date to the "YYYY-MM-DDTHH:MM" format the
// input expects, using the *local* clock — no manual offset arithmetic.
...
// Human-readable preview shown below each datetime input.
...
export default function CreateExamPage() {
  ...
  // min values for datetime inputs — always in local time
    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-4 border-b pb-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Configure timing, AI proctoring rules, and student invitations.</p>
        </div>
      </div>
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <FormField control={form.control} name="start_window" render={({ field }) => (
                <FormItem>
                  ...
                  {field.value && (
                    <p className="text-xs text-slate-500 mt-1">{formatDatePreview(field.value)}</p>
                  )}
                  <FormMessage />
                </FormItem>
              )} />
...
Just raw text with email@uni.edu inside`}
          ...
          <div className="flex justify-end gap-4">
            ...
            <Button type="submit" disabled={isSubmitting} className="bg-indigo-600 hover:bg-indigo-700 w-36 shadow-md">
              {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Publish Exam'}
            </Button>
          </div>
