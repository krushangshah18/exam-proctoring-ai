...
import { Camera, User, Lock, Loader2, RefreshCw, CheckCircle2, X, Mail, Shield, Eye, EyeOff, GraduationCap } from 'lucide-react';
...
function FieldGroup({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: '20px' }}>
      <label style={{ display: 'block', fontSize: '12.5px', fontWeight: 700, color: '#475569', marginBottom: '7px', letterSpacing: '0.01em' }}>
        {label}
      </label>
      {children}
...
function SectionHeader({ icon, title, subtitle }: { icon: React.ReactNode; title: string; subtitle: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '22px', paddingBottom: '16px', borderBottom: '1.5px solid #F1F5F9' }}>
      <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: 'rgba(34,87,122,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        {icon}
      </div>
      <div>
        <h3 style={{ fontSize: '14.5px', fontWeight: 800, color: '#0F172A', letterSpacing: '-0.01em' }}>{title}</h3>
        <p style={{ fontSize: '12px', color: '#64748B', marginTop: '1px', fontWeight: 500 }}>{subtitle}</p>
      </div>
    </div>
  );
}
...
export default function StudentProfile() {
  ...
  const handleChangePassword = async () => {
    if (!passwordData.old_password || !passwordData.new_password) { toast.error('Please fill in all password fields'); return; }
    ...
    setUpdating(true);
    try {
      await api.post('/auth/change-password', { old_password: passwordData.old_password, new_password: passwordData.new_password });
  ...
  const handleProfileImageUpload = async () => {
    if (!imgSrc) { toast.error('Please capture a selfie first'); return; }
    setUpdating(true);
    try {
      ...
      await api.put('/auth/me/profile-image', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
      toast.success('Profile image updated successfully');
      setShowWebcam(false);
      setImgSrc(null);
      fetchUserProfile();
    } catch (e: any) { toast.error(e.response?.data?.detail || 'Failed to update profile image'); }
    finally { setUpdating(false); }
  };
      ...
      <StudentPageShell backHref="/student/dashboard" backLabel="Back to Dashboard">
        ...
        <div style={{ maxWidth: '900px', animation: 'fadein 0.4s ease both' }}>
          ...
          <div style={{
            ...
            <div style={{
              height: '110px',
              background: 'linear-gradient(135deg, #22577A 0%, #38A3A5 60%, #57CC99 100%)',
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginTop: '-36px', flexWrap: 'wrap', gap: '12px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: '16px' }}>
                  ...
                  <div style={{
                    ...
                    {user?.profile_image_path ? (
                      ...
                      <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', fontWeight: 800, color: '#fff' }}>
                        {initials}
                      </div>
                    )}
                  </div>
                  ...
                  <div style={{ paddingBottom: '4px' }}>
                    ...
                    <p style={{ fontSize: '13px', color: '#64748B', fontWeight: 500, marginTop: '3px', display: 'flex', alignItems: 'center', gap: '5px' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: 'rgba(34,87,122,0.07)', border: '1.5px solid rgba(34,87,122,0.18)',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
            ...
            <div className="pf-card">
              <SectionHeader
                icon={<User style={{ width: '16px', height: '16px', color: '#22577A' }} />}
                title="Basic Information"
                subtitle="Update your name and account details"
              />

              <FieldGroup label="Email Address" hint="Email cannot be changed after registration">
              ...
              <button
                ...
                disabled={updating}
                style={{
                  width: '100%', padding: '11px', borderRadius: '9px', border: 'none', cursor: updating ? 'not-allowed' : 'pointer',
                  background: updating ? '#94A3B8' : '#22577A', color: '#fff',
                ...
                onMouseEnter={e => { if (!updating) (e.currentTarget as HTMLElement).style.background = '#2C6A91'; }}
                onMouseLeave={e => { if (!updating) (e.currentTarget as HTMLElement).style.background = '#22577A'; }}
              >
                {updating
                  ? <><Loader2 style={{ width: '14px', height: '14px', animation: 'spin 1s linear infinite' }} /> Saving…</>
                  : <><CheckCircle2 style={{ width: '14px', height: '14px' }} /> Save Changes</>
                }
              </button>
            </div>
            ...
            <div className="pf-card">
              <SectionHeader
                icon={<Lock style={{ width: '16px', height: '16px', color: '#22577A' }} />}
              ...
              <FieldGroup label="Confirm New Password">
                <div style={{ position: 'relative' }}>
                  <input
                    ...
                    className="pf-input"
                    style={{
                      ...inputStyle, paddingRight: '38px',
                      borderColor: passwordData.confirm_password && passwordData.confirm_password !== passwordData.new_password ? 'rgba(239,68,68,0.5)' : undefined,
                    }}
                    onFocus={e => { e.currentTarget.style.borderColor = '#38A3A5'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(56,163,165,0.1)'; }}
                    onBlur={e => {
                      const mismatch = passwordData.confirm_password && passwordData.confirm_password !== passwordData.new_password;
                      e.currentTarget.style.borderColor = mismatch ? 'rgba(239,68,68,0.5)' : '#E2E8F0';
                ...
                {passwordData.confirm_password && passwordData.confirm_password !== passwordData.new_password && (
                  <p style={{ fontSize: '11.5px', color: '#EF4444', marginTop: '4px', fontWeight: 600 }}>Passwords do not match</p>
                )}
              </FieldGroup>
              ...
              <button
                ...
                style={{
                  width: '100%', padding: '11px', borderRadius: '9px', border: 'none', cursor: updating ? 'not-allowed' : 'pointer',
                  background: updating ? '#94A3B8' : '#0F172A', color: '#fff',
                  fontSize: '13.5px', fontWeight: 700,
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '7px',
                  fontFamily: "'Plus Jakarta Sans', sans-serif",
                  transition: 'background 150ms',
                ...
                onMouseEnter={e => { if (!updating) (e.currentTarget as HTMLElement).style.background = '#1E293B'; }}
                onMouseLeave={e => { if (!updating) (e.currentTarget as HTMLElement).style.background = '#0F172A'; }}
              >
                {updating
                  ? <><Loader2 style={{ width: '14px', height: '14px', animation: 'spin 1s linear infinite' }} /> Updating…</>
                  : <><Lock style={{ width: '14px', height: '14px' }} /> Update Password</>
                }
              </button>
            </div>
          </div>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div style={{ width: '32px', height: '32px', borderRadius: '8px', background: 'rgba(34,87,122,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Camera style={{ width: '15px', height: '15px', color: '#22577A' }} />
                  </div>
                  <div>
                    <h3 style={{ fontSize: '14.5px', fontWeight: 800, color: '#0F172A', letterSpacing: '-0.01em' }}>Capture Selfie</h3>
                    <p style={{ fontSize: '12px', color: '#64748B', fontWeight: 500, marginTop: '1px' }}>Used for exam face verification</p>
                  </div>
                </div>
                <button
                  onClick={() => { setShowWebcam(false); setImgSrc(null); }}
                  style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '6px', borderRadius: '8px', color: '#94A3B8', display: 'flex', transition: 'background 150ms' }}
              ...
              <div style={{ padding: '20px 22px' }}>
                <div style={{
                  ...
                  {!imgSrc ? (
                    ...
                    <>
                      <img src={imgSrc} alt="Selfie" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                      <div style={{ position: 'absolute', top: '10px', right: '10px', background: 'rgba(34,197,94,0.9)', padding: '3px 10px', borderRadius: '20px', display: 'flex', alignItems: 'center', gap: '5px' }}>
                        <CheckCircle2 style={{ width: '11px', height: '11px', color: '#fff' }} />
                        <span style={{ fontSize: '10px', fontWeight: 700, color: '#fff' }}>Photo Captured</span>
                      </div>
                    </>
                  )}
                </div>
                  ...
                  {!imgSrc ? (
                    <button
                      ...
                      style={{
                        ...
                        background: '#fff', color: '#22577A', fontSize: '13px', fontWeight: 700,
                        cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif",
                        transition: 'all 150ms',
                      }}
                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#38A3A5'; (e.currentTarget as HTMLElement).style.background = '#F0FDFC'; }}
                    ...
                    >
                      <Camera style={{ width: '14px', height: '14px' }} /> Take Photo
                    </button>
                  ) : (
                    <button
                    ...
                    >
                      <RefreshCw style={{ width: '13px', height: '13px' }} /> Retake
                    </button>
                  )}
                </div>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  <button
                    onClick={() => { setShowWebcam(false); setImgSrc(null); }}
                    style={{
                      flex: 1, padding: '11px', borderRadius: '9px', border: '1.5px solid #E2E8F0',
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: !imgSrc || updating ? '#94A3B8' : '#22577A',
                    ...
                    onMouseEnter={e => { if (imgSrc && !updating) (e.currentTarget as HTMLElement).style.background = '#2C6A91'; }}
                    onMouseLeave={e => { if (imgSrc && !updating) (e.currentTarget as HTMLElement).style.background = '#22577A'; }}
                  >
                    {updating
                      ? <><Loader2 style={{ width: '14px', height: '14px', animation: 'spin 1s linear infinite' }} /> Uploading…</>
                      : <><CheckCircle2 style={{ width: '14px', height: '14px' }} /> Save Photo</>
                    }
                  </button>
                </div>
              </div>
