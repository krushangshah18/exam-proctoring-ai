    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
          ...
          <div>
            <h3
              style={{ fontSize: "14.5px", fontWeight: 700, color: "#0F172A" }}
            >
              {title}
            </h3>
            <p
              style={{ fontSize: "12.5px", color: "#64748B", marginTop: "2px" }}
            >
              {subtitle}
            </p>
          </div>
...
export default function SystemCheckPage() {
  ...
  useEffect(() => {
    const initSystem = async () => {
      try {
        ...
        const isApprovalResuming =
          data.active_resume_request?.status === "APPROVED";
        if (
          !isReconnect &&
          !isApprovalResuming &&
          data.hard_join_deadline &&
          data.session_status !== "ACTIVE"
        ) {
          const deadlineMs = parseUTC(data.hard_join_deadline).getTime();
          if (Date.now() > deadlineMs) {
            toast.error(
              "The join deadline has passed. Please submit an appeal.",
            );
            router.replace(`/student/exam/${examId}/info`);
            return;
          }
          ...
          if (msUntilStart > 0) {
            ...
            countdownIntervalRef.current = setInterval(() => {
              const remaining = Math.max(
                0,
                Math.ceil((countdownEndRef.current - Date.now()) / 1000),
              );
  ...
  const runSystemChecks = async () => {
    ...
    setTimeout(() => {
      ...
      if (!document.fullscreenEnabled)
        toast.error(
          "Your browser does not support fullscreen mode. Please use Chrome or Edge.",
        );
    }, 1200);
    ...
    try {
      ...
      if (res.data.available) {
        ...
        setEngineError(
          "Please wait a few minutes — we are allocating a proctoring engine for your session.",
        );
        toast.error(
          "Proctoring engine is being allocated. Please retry in a moment.",
        );
      }
    } catch {
      setEngineStatus("fail");
  ...
  const handleStartExam = async () => {
    try {
      ...
      const isLateJoinError =
        err?.response?.status === 403 &&
        (detail.includes("Late join") ||
          detail.includes("appeal") ||
      ...
      if (isLateJoinError) {
        ...
        toast.error(
          detail || "Could not launch exam. Ensure browser allows Fullscreen.",
        );
      }
    }
  };
  ...
  const allPassed =
    fsStatus === "pass" && sessionStatus === "pass" && engineStatus === "pass";
  const anyChecking =
    fsStatus === "checking" ||
    sessionStatus === "checking" ||
    engineStatus === "checking";
  const anyFailed =
    fsStatus === "fail" || sessionStatus === "fail" || engineStatus === "fail";
      ...
      >
        ...
        >
          <div style={{ textAlign: "center", marginBottom: "28px" }}>
            ...
            >
              <ShieldAlert
                style={{ width: "24px", height: "24px", color: "#22577A" }}
              />
            </div>
            <h1
              style={{
          ...
          {anyFailed && (
            ...
            >
              <Loader2 style={{ width: "14px", height: "14px" }} /> Retry All
              Checks
            </button>
          )}
