      ...
      while (!cancelled) {
        ...
        for (const line of lines) {
          if (line.startsWith("event: ")) eventType = line.slice(7).trim();
            ...
            try {
              handlers[eventType]?.(JSON.parse(line.slice(6)));
            } catch {}
            eventType = "message";
          }
        }
...
export default function ActiveExamPage() {
  ...
  const tabThreshold = exam?.config?.tab_switching
    ? exam?.tab_switch_terminate_count || 3
    : Infinity;
  const tabThresholdRef = useRef<number>(Infinity);
  useEffect(() => {
    tabThresholdRef.current = tabThreshold;
  }, [tabThreshold]);
  ...
  const executeSubmit = useCallback(
    async (isAuto = false) => {
      ...
      try {
        ...
        toast.error(
          err?.response?.data?.detail || "Submit failed. Please retry.",
        );
    ...
    (sessionId: string) => {
      ...
      sseCloseRef.current = openSSE(`${baseUrl}/exam/${examId}/events`, token, {
        TIME_EXTENDED: (data) => {
          pushAlert(
            "success",
            `Time extended by ${data.added_minutes} minute(s)!`,
          );
        ...
        IDENTITY_PROMPT: () => {
          ...
          if (identityPromptTimerRef.current)
            clearTimeout(identityPromptTimerRef.current);
          identityPromptTimerRef.current = setTimeout(
  ...
  const setupProctor = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const hasEndedTracks = streamRef.current
        .getTracks()
        ...
        token,
        {
          message: (data: any) => {
            ...
            if (t === "alert" || t === "warning") {
              pushAlert(
                "warning",
                data.message || data.alert_type || "Proctoring alert",
              );
            } else if (t === "risk_update") {
              if (data.state === "HIGH_RISK" || data.state === "ADMIN_REVIEW") {
                pushAlert(
                  "critical",
                  `Risk Level: ${data.state} (score: ${Math.round(data.risk_score ?? 0)})`,
                );
              }
            }
          },
      ...
      if (
        ...
        err?.response?.data?.detail === "MODEL_CALIBRATION_ACTIVE"
      ) {
    ...
    const init = async () => {
      try {
        const [sessionRes] = await Promise.all([
          api.get(`/exam/${examId}/session-active`),
        ]);
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    const onFsChange = async () => {
      if (!document.fullscreenElement && !terminated && !submitting) {
        ...
        try {
          const res = await api.post(`/exam/${examId}/proctor-violation`, {
  ...
  const reportSwitch = useCallback(async () => {
    ...
    pushAlert(
      "critical",
      `Tab switch detected (${next}/${limit === Infinity ? "∞" : limit} allowed)`,
    );
    try {
      const res = await api.post(`/exam/${examId}/proctor-violation`, {
        reason: "tab_switch",
  ...
  return (
    ...
    >
      ...
      {(proctorStatus === "connecting" || proctorStatus === "failed") &&
        !terminated && (
          <div
            style={{
              position: "absolute",
          ...
          >
            <div
              style={{
                background: "#fff",
                border: `2px solid ${proctorStatus === "failed" ? "#EF4444" : "#22577A"}`,
                borderRadius: "20px",
                padding: "40px",
                maxWidth: "400px",
                width: "100%",
            ...
            >
                ...
                style={{
                  ...
                  background:
                    proctorStatus === "failed"
                      ? "rgba(239,68,68,0.1)"
                      : "rgba(34,87,122,0.1)",
                  border: `2px solid ${proctorStatus === "failed" ? "rgba(239,68,68,0.25)" : "rgba(34,87,122,0.25)"}`,
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  margin: "0 auto 18px",
              ...
              >
                {proctorStatus === "connecting"
                  ? "Connecting to Proctoring Engine…"
                  : "Proctoring Engine Unavailable"}
              </h2>
              <p
                style={{
                  fontSize: "13.5px",
                  color: "#64748B",
                  marginBottom: proctorStatus === "failed" ? "24px" : "0",
                  lineHeight: 1.65,
                  fontWeight: 500,
                }}
              >
              ...
              {proctorStatus === "failed" && (
                ...
                >
                  <Shield style={{ width: "16px", height: "16px" }} /> Retry
                  Connection
                </button>
              )}
            </div>
      ...
      {fullscreenExited && !terminated && (
        ...
        >
          ...
          >
            ...
            >
              You exited fullscreen mode. The exam must be in fullscreen at all
              times. This will be recorded as a critical event.
            </p>
            <button
              onClick={() => {
                document.documentElement.requestFullscreen().catch(() => {});
            ...
            >
              <Maximize2 style={{ width: "16px", height: "16px" }} /> Return to
              Fullscreen
            </button>
          </div>
        </div>
      ...
      {calibrationBlocked && (
        ...
        >
          ...
          />
          <div
            style={{ textAlign: "center", color: "#fff", maxWidth: "380px" }}
          >
            <div
            ...
            >
              The proctoring engine is currently being calibrated by an
              administrator. Your exam session is preserved — please wait and
              try again shortly.
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div
            style={{
              ...
              animation: isLowTime
                ? "pulse-timer 1s ease-in-out infinite"
                : "none",
            }}
          >
            <Clock
      ...
      <div
        ...
        onCopy={(e) => {
          e.preventDefault();
          pushAlert("warning", "Copying is not allowed during the exam.");
        }}
        onPaste={(e) => {
          e.preventDefault();
          pushAlert("warning", "Pasting is not allowed during the exam.");
        }}
        onCut={(e) => {
          e.preventDefault();
          pushAlert("warning", "Cutting text is not allowed during the exam.");
        }}
      >
        {/* Question canvas */}
            ...
            >
              ...
              <div>
                ...
                >
                  ...
                  >
                    Exam question content renders here in the full
                    implementation. This is a sample question to demonstrate the
                    exam interface and copy-paste restrictions.
                  </p>
                </div>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <textarea
                    ...
                    onPaste={(e) => {
                      ...
                      pushAlert(
                        "warning",
                        "Pasting is not allowed during the exam.",
                      );
                    }}
                    onCut={(e) => {
                      e.preventDefault();
                      pushAlert(
                        "warning",
                        "Cutting text is not allowed during the exam.",
                      );
                    }}
                    style={{
                      width: "100%",
                  ...
                  >
                    Keyboard shortcuts Ctrl+C, Ctrl+V, and Ctrl+X are blocked.
                    Any attempt is logged.
                  </p>
                </div>
              </div>
            </div>
