...
import {
  Cpu, MemoryStick, Zap, Server, RefreshCw,
  AlertTriangle, CheckCircle2, XCircle, Loader2, ChevronDown, ChevronUp,
  Play, Square, Activity, Power, Edit2, X, Check, CloudLightning,
} from 'lucide-react';
...
function EC2ConfigDialog({
  containerId, current, onClose, onSaved,
}: {
  containerId: string;
  current: { container_name?: string | null; container_port?: number | null; ec2_instance_id?: string | null; ec2_region?: string | null };
  onClose: () => void;
  onSaved: () => void;
}) {
  ...
  const handleSave = async () => {
    if (!form.container_name.trim()) { toast.error('Container name is required'); return; }
    setSaving(true);
    try {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.5)' }}>
      <div className="bg-white rounded-xl border shadow-xl w-full max-w-md p-6" style={{ borderColor: '#E2E8F0' }}>
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-base font-bold" style={{ color: '#0F172A' }}>Configure Container</h3>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider mb-1.5" style={{ color: '#64748B' }}>EC2 Instance ID</label>
            <input type="text" placeholder="i-0abc123def456789"
              value={form.ec2_instance_id}
              onChange={e => setForm(f => ({ ...f, ec2_instance_id: e.target.value }))}
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider mb-1.5" style={{ color: '#64748B' }}>AWS Region</label>
            <input type="text" placeholder="ap-south-1"
              value={form.ec2_region}
              onChange={e => setForm(f => ({ ...f, ec2_region: e.target.value }))}
              className="w-full px-3 py-2 rounded-lg border text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500"
              style={{ borderColor: '#E2E8F0', color: '#0F172A' }} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider mb-1.5" style={{ color: '#64748B' }}>Container Name</label>
              <input type="text" placeholder="proctor1"
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider mb-1.5" style={{ color: '#64748B' }}>Port</label>
              <input type="number" placeholder="8000"
                value={form.container_port}
                onChange={e => setForm(f => ({ ...f, container_port: parseInt(e.target.value) || 8000 }))}
                className="w-full px-3 py-2 rounded-lg border text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500"
                style={{ borderColor: '#E2E8F0', color: '#0F172A' }} />
            </div>
          </div>
...
function EC2InstanceCard({ containers, onRefresh }: { containers: ContainerInfo[]; onRefresh: () => void }) {
  ...
  const [actionLoading, setActionLoading] = useState<'start' | 'stop' | 'apply' | null>(null);
  ...
  const fetchStatus = async () => {
    ...
    try {
      const r = await api.post('/admin/engine/ec2/status', { instance_id: instanceId, region });
      setEc2State(r.data);
    } catch (e: any) {
  ...
  const handleStart = async () => {
    ...
    try {
      await api.post('/admin/engine/ec2/start', { instance_id: instanceId, region });
  ...
  const handleStop = async () => {
    ...
    try {
      await api.post('/admin/engine/ec2/stop', { instance_id: instanceId, region });
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      toast.success(r.data.message || 'Settings applied. Containers restarting…');
      setTimeout(() => { fetchStatus(); onRefresh(); }, 5000);
    } catch (e: any) {
  ...
  const stateColor = ec2State?.state === 'running'  ? '#15803D'
    : ec2State?.state === 'stopped'                 ? '#BE123C'
    : ec2State?.state === 'pending' || ec2State?.state === 'stopping' ? '#D97706'
    : '#64748B';

  const stateBg = ec2State?.state === 'running'  ? { background: '#ECFDF5', color: '#15803D', borderColor: '#BBF7D0' }
    : ec2State?.state === 'stopped'               ? { background: '#FFF1F2', color: '#BE123C', borderColor: '#FECDD3' }
    : ec2State?.state                             ? { background: '#FFFBEB', color: '#D97706', borderColor: '#FDE68A' }
  ...
  return (
    <div className="bg-white rounded-xl border p-5 space-y-4"
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {ec2State && (
          <span className="px-2.5 py-1 rounded-lg border text-xs font-semibold" style={stateBg}>
      ...
      {displayId && (
        <div className="grid grid-cols-3 gap-3 text-xs">
          <div className="rounded-lg border p-3" style={{ background: '#F8FAFC', borderColor: '#E2E8F0' }}>
            ...
            <p className="font-mono font-semibold" style={{ color: '#0F172A' }}>{region}</p>
          </div>
          <div className="rounded-lg border p-3" style={{ background: '#F8FAFC', borderColor: '#E2E8F0' }}>
          ...
          <div className="rounded-lg border p-3" style={{ background: '#F8FAFC', borderColor: '#E2E8F0' }}>
            ...
            <p className="font-semibold" style={{ color: '#0F172A' }}>{containers.filter(c => c.container_name).map(c => c.container_name).join(', ') || '—'}</p>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <button onClick={handleApply} disabled={!!actionLoading}
          ...
          {actionLoading === 'apply' ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Power className="h-3.5 w-3.5" />}
        ...
        <div className="space-y-1.5">
          {applyResults.map((r, i) => (
            <div key={i} className="flex items-center gap-3 px-3 py-2 rounded-lg border text-xs"
              ...
              {r.status === 'sent' ? <CheckCircle2 className="h-3.5 w-3.5 shrink-0" /> : <XCircle className="h-3.5 w-3.5 shrink-0" />}
              <span className="font-mono font-semibold">{r.container}</span>
              {r.status === 'sent' && r.command_id && (
                <span className="ml-auto text-[10px] font-mono opacity-60">{r.command_id}</span>
              )}
...
function ContainerCard({
  container, metricsResult, onRefresh,
}: {
        ...
        <div className="flex items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <p className="font-mono text-sm font-semibold truncate" style={{ color: '#0F172A' }}>{container.url}</p>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {!metricsResult ? (
              <span className="px-2 py-0.5 rounded-md text-xs font-semibold border"
                style={{ background: '#F8FAFC', color: '#64748B', borderColor: '#E2E8F0' }}>Fetching…</span>
            ) : metricsResult.ok ? (
            ...
            <button onClick={() => setExpanded(e => !e)}
              className="p-1 rounded-lg transition-colors" style={{ color: '#64748B' }}>
                    ...
                    <>
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between text-xs">
                          ...
                          <span className="font-semibold tabular-nums" style={{ color: '#DB2777' }}>
                            {fmt(sys.gpu_mem_used_mb, 0)} / {fmt(sys.gpu_mem_total_mb, 0)} MB
                          </span>
                        </div>
...
export default function EngineMonitorPage() {
  ...
  return (
    <div className="space-y-6 max-w-6xl pb-12">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm mt-1 font-medium" style={{ color: '#64748B' }}>
            Real-time proctoring engine health, resource usage, and EC2 management
          </p>
        </div>
      ...
      {/* EC2 Instance Card */}
      <EC2InstanceCard containers={containers} onRefresh={() => fetchAll(true)} />

      {/* Container cards */}
      {containers.length === 0 ? (
        <div className="bg-white rounded-xl border p-16 text-center" style={{ borderColor: '#E2E8F0' }}>
